...
function addGamesToPage(games) {
    ...
    for(let i = 0; i < games.length; i++){
        ...
        gameCard.innerHTML = `
            <img src="${currentGame.img}" alt="Image of ${currentGame.name}" />
            <h3>${currentGame.name}</h3>
            <p>${currentGame.description || 'No description available.'}</p>
            `;
...
const totalIndividualContributions = GAMES_JSON.reduce((acc, game) => {
...
console.log(showAllGames().length);
...
const description = totalUnfundedGames > 0 ? `${totalUnfundedGames} games are still unfunded.` : "All games are funded.";

// create a string that explains the number of unfunded games using the ternary operator
const newParagraphElement = document.createElement('p');
newParagraphElement.textContent = description;
